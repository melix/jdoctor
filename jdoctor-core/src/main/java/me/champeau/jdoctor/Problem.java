 ...
 *
 * <p>A {@link StandardSeverity convenience, opiniated, severity enum} is available for use
 * if you need one.</p>
 *
 * @param <ID> an ID. Each problem should be uniquely identified by ID.
 * @param <SEVERITY> the severity type
 * @param <CONTEXT> the type of the context, representing where the problem happened
 */
public interface Problem<ID, SEVERITY extends Enum<SEVERITY>, CONTEXT> extends WithId<ID>, WithDescription, WithDocumentationLink {
    /**
